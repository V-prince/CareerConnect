...
import { GetAdminManangeDataApI } from "../../Services/adminService";
...
export const ManageUsers = () => {
  ...
  const UserCounts = [
    {
      icon: <Briefcase />,
      title: "Candidates",
      count: totalUsers.find(item => item._id === "candidate")?.count || 0,
      bgColor: "bg-blue-500",
    },
    {
      icon: <Building />,
      title: "Employeer",
      count: totalUsers.find(item => item._id === "employer")?.count || 0,
      bgColor: "bg-green-500",
    },
    {
      icon: <ShieldKeyhole />,
      title: "Admin",
      count: totalUsers.find(item => item._id === "admin")?.count || 0,
      bgColor: "bg-red-500",
    },
  ];
  ...
  const handelOnFielterd = () => {
    ...
    if (details.search.trim()) {
      Fielterd = Fielterd.filter(
        (user) =>
          ...
          String(user?.phone || "").includes(details.search)
      );
    }
    ...
    if (details.sort === "new") {
      Fielterd.sort((a, b) => new Date(b?.createdAt) - new Date(a?.createdAt))
    }
  ...
  const getData = async () => {
    try {
      ...
      const data = await GetAdminManangeDataApI();
      if (!data.success) {
        return console.log(data.message)
      }
      setUsers(data?.users)
      setFilterdDetails(data?.users)
  ...
  return (
    <section className="w-full mt-16 px-4 sm:px-6 lg:px-8 py-6">
              ...
              <tbody>
                {filterdDetails.map((user, index) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        {
                          ...
                          user.photo ? (
                            ...
                            <p
                              className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-blue-100 font-semibold text-blue-500 text-center p-2"
                            >
                              {user?.fullname.split(" ").map(item => item[0]).join("").toUpperCase()}
                            </p>
                          )
                        }

                        <div className="min-w-0">
                          <h1 className="font-semibold text-sm sm:text-base truncate max-w-[180px] capitalize">
                            {user?.fullname}
                          </h1>
                    ...
                    <td className="text-center text-sm capitalize">
                      {user?.role ? user.role : "-"}
                    </td>
                    ...
                    <td className="relative text-center px-4">
                      ...
                      {isOpenPopup && selectedUser === user._id && (
                        <div className="absolute right-2 top-12 z-50 w-36 sm:w-40 bg-white border border-zinc-200 rounded-lg shadow-lg p-1  flex flex-col items-center  ">
