...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${styles[status] || "bg-zinc-100 text-zinc-600"
        }`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${status === "open"
          ? "bg-green-500"
          : status === "expired"
            ? "bg-red-500"
            : "bg-zinc-400"
...
const EmpJobsTable = ({ user, currentJobs, onEdit, onView, onDelete }) => {
